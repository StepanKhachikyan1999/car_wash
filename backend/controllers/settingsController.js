...
const confirmCountersReset = async (req, res) => {
    try {
        ...
        console.log(token,id,'7777777777777777');
        const data = {
            "id": [0]
        }
        ...
        (async () => {
            console.log(sub,"88888888888888888888");
            for await(const m of sub) {
                console.log(m);
                let response = JSON.parse(sc.decode(m.data))
                console.log("🚀 ~ file: settingsController.js ~ line 308 ~ forawait ~ response", response)
...
const receiveDateTime = async (req, res) => {
    try {
        ...
        let date = new Date().toISOString().replace('-', '').replace('-', '')
